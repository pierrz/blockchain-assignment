...
import { transactionTableName, transactionCSVFields } from '../models/transactions.js';
export async function importTransactions() {
    async function processTransactions(filePath) {
        ...
        let transactions = [];
        try {
            await pipeline(createReadStream(filePath), createGunzip(), parse({
    ...
    const dataDir = config.get('directories.dataDir'), sourceDir = join(dataDir, config.get('directories.sourceDir'));
    importData(sourceDir, transactionTableName, processTransactions).catch((error) => {
