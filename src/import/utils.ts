...
export async function importData(
  ...
  const dataDir = config.get<string>("directories.dataDir"),
    processedDir = join(
      dataDir,
      config.get<string>("directories.processedDir"),
    ),
  ...
  try {
      ...
      try {
        ...
        if (processedData.length > 0) {
          try {
            ...
            console.log(
              `Inserted ${processedData.length} ${tableName} from ${file}`,
            );
            ...
            try {
              ...
              console.log(
                "File moved to the 'failed' directory due to insertion error.",
              );
            } catch (moveError) {
              console.error(
    ...
    if (totalProcessed === 0) {
      ...
      console.log(
        `Import completed successfully. Total ${tableName} processed: ${totalProcessed}`,
      );
    }
  } catch (error) {
