export async function monitorEvents(client: any, tableName: string, eventTypeReference: any, processFunction: CallableFunction, insertFunction: CallableFunction) {
  ...
  try {
    ...
    const unwatch = client.watchBlocks({
      onBlock: async (block: any) => {
        ...
        const validTxs = processedTxs.filter((tx): tx is typeof eventTypeReference => 
          tx !== null && 
